...
from hitfloor.report.summary import write_batch_aware_summary, write_phase1_summary
from hitfloor.report.tables import write_csv_table
...
class ExperimentRunner:
    ...
    def run(self) -> ExperimentResult:
        ...
        if mode == "infinite_hbm":
            return self._run_infinite_hbm(output_dir=output_dir, requests=requests)
        if mode == "batch_aware_infinite_hbm":
            return self._run_batch_aware_infinite_hbm(
                output_dir=output_dir,
                requests=requests,
            )
        if mode == "batch_aware_hbm_lru":
            return self._run_batch_aware_hbm_lru(
                output_dir=output_dir,
                requests=requests,
            )
        raise ValueError(f"unsupported simulation mode: {mode}")

    def _build_requests(self) -> list[SimulationRequest]:
        return build_requests_from_config(self.config)
