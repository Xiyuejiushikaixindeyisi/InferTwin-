...
from hitfloor.cache.event_sink import CacheEventSink, CacheEventStats, StatsOnlyCacheEventSink
from hitfloor.cache.eviction import LRUEvictor
...
from hitfloor.report.cache_events import CsvCacheEventWriter
...
class CapacitySweepRunner:
    ...
    def run(self) -> CapacitySweepResult:
        requests = build_requests_from_config(self.config)
        ...
        return CapacitySweepResult(
            ...
            config_details=self._config_details(),
    ...
    def _config_details(self) -> Mapping[str, object]:
...
def build_capacity_sweep_config(config: Mapping[str, Any]) -> CapacitySweepConfig:
    """Validate and normalize the Step6 sweep config section."""

    mode = _mapping(config, "simulation").get("mode")
    if mode != "capacity_sweep":
        raise ValueError("capacity_sweep requires simulation.mode: capacity_sweep")
