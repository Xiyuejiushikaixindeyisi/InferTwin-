...
| L1 | 当前阶段文档，例如未来 `docs/stepY/` | 当前阶段范围、技术路线、batch 计划 | 是 |
...
| materialization timing | legacy mode 默认 finish-time materialization；Step9 progressive mode 支持 chunk finish 后 full-block materialization | legacy mode 冻结旧语义；新语义通过新 mode 承载 | 后续按新 mode 扩展 | 需要 partial-block hit、promotion 或 physical slot/refcount 时 |
| progressive block visibility | 已通过 `batch_aware_hbm_ddr_lru_progressive_timeline` 新 mode 启用；partial block 仍不可见 | 保持 legacy mode 不变，同时补齐 V1 长 prefill 复用低估问题 | 已完成 V1 目标，后续可增强 | 需要 per-chunk dump、partial hit 或更真实 transfer timeline 时 |
...
- Step9：progressive chunk visibility / chunk-level TTFT timeline 已完成；chunk finish 后 newly completed full blocks 可以成为后续请求的 cache-hit 候选。
