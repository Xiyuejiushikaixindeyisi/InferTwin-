...
对 InferTwin Step4 来说，MkSim 的定位是：
...
2. InferTwin adapter 自己循环调用单 shape，例如循环 `Task(...).execute()`。
...
当前 InferTwin `BatchShape.batch_size = request slice 数` 与 MkSim 的 batch size 在语义上基本一致，前提是：

```text
一个 ScheduledSlice 表示一个请求在一次 iteration 中的一段 prefill work
```

但 MkSim 的 workload shape 是单一 `seq_len/prefix_cache/batch_size`，这要求同一个 MkSim 调用中的 batch 请求具有统一 shape。InferTwin scheduler 可能生成 heterogeneous batch：

- 不同 request 的 `scheduled_prefill_tokens` 不同。
- 不同 request 的 `computed_tokens_before` 不同。
- 不同 request 的 `cached_tokens` 不同。

因此 InferTwin 不能简单把任意 `BatchShape` 直接传给 MkSim。需要在 adapter 层显式设计转换策略。
...
- `scheduled_decode_tokens` 在 InferTwin 里仍为 0。
...
对 InferTwin 来说，一个 chunk 应表达为一次 prefill shape：
...
对 InferTwin 更合适的是后者，因为 InferTwin 需要 TTFT 中的 compute 部分尽量反映已缓存前缀对 attention/KV memory 的影响。
...
cache hit/miss generation: InferTwin
```
...
InferTwin `BatchShape` 是 scheduler output，包含每个 request slice。
...
InferTwin scheduler 可能产生 heterogeneous batch，MkSim 单次调用不直接支持。
...
InferTwin chunked prefill 的一个 slice 应转换为：
...
MkSim workload 需要 `out_len`。InferTwin Step4 不建模 decode TPOT。
...
1. InferTwin 是否为 MkSim adapter 采用 internal API，而不是 CLI。
...
6. AIConfigurator 是否支持 per-request heterogeneous batch；如果支持，InferTwin 公共接口应向 AIConfigurator 靠拢还是向 MkSim uniform shape 靠拢。
