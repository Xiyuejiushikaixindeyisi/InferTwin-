...
3. 明确当前骨架是否可以作为后续 HitFloor 扩展基础，以及进入下一阶段前建议优先处理的问题。
...
- P1 package CLI 占位问题已修复：`src/hitfloor/cli/main.py` 已调用真实 runner / trace reader，scripts 已退为 wrapper。
...
但它还不是完整的 HitFloor 产品形态。当前主要是“核心仿真骨架”，还缺：
...
src/hitfloor/request/chat_template.py:7: F401 typing.Any imported but unused
```

说明：

- `E402` 来自脚本中手动插入 `src` 到 `sys.path` 后再 import HitFloor 模块。
...
```bash
TMPDIR=/tmp HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 PYTHONPATH=src .venv/bin/python -m pytest --cov=hitfloor --cov-report=term-missing
```
...
### P1. 包安装后的 `hitfloor` CLI 仍是占位实现

证据：

- `pyproject.toml` 注册了 `hitfloor = "hitfloor.cli.main:main"`。
- `src/hitfloor/cli/main.py` 的 `simulate` 分支只打印 `"Simulation entry is ready"`，没有调用 `load_yaml`、`ExperimentRunner` 或写报告。
- 当前真实可用入口是 `scripts/run_simulation.py`。

影响：

- 同事如果按 Python package 方式安装并执行 `hitfloor simulate --config ...`，不会真正跑仿真。
- 这会造成“项目可运行”与“包命令可运行”的体验不一致。

建议：

- 将 `scripts/run_simulation.py` 的真实逻辑迁入 `hitfloor.cli.main`。
- `scripts/run_simulation.py` 可以保留为薄 wrapper。
- 为 `hitfloor simulate` 和 `hitfloor validate-trace` 增加 integration tests。
...
- `src/hitfloor/experiment/search.py` 只有简单 grid helper，没有接入 runner/report。

影响：

- 当前输出是 replay metrics，不是“不同 P90 TTFT 对应底线 KV cache hit”的最终产品表。
- Step1-Step5 完成的是仿真骨架，不是完整 HitFloor 产品闭环。
...
- 不建议直接在现有 `ExperimentRunner._run_batch_aware_hbm_lru()` 中堆搜索逻辑，应新增 `HitFloorSearchRunner` 或 `experiment/search_runner.py`。
...
- 如果现网数据出现缺失 model、message item 非 dict、role/content 结构异常，HitFloor 可能继续运行并给出看似正常的结果。
...
```text
HitFloor Step1-Step5 provides a maintainable offline replay skeleton,
...
完成后，HitFloor 会更适合交给同事接入外部 simulator、跑更大 trace、继续开发 hit floor search。
